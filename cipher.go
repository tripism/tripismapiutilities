...
func EncryptString(plainText, cipherKey string) (cipherTextString string, err error) {
	key := hashTo32Bytes(cipherKey)
...
func DecryptString(cryptoText, cipherKey string) (plainTextString string, err error) {
	...
	decrypted, err := decryptAES(hashTo32Bytes(cipherKey), encrypted)
	if err != nil {
		return "", err
	}
